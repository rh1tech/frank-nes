...
static uint8_t parity_table[32] = {0x96, 0x69, 0x69, 0x96, 0x69, 0x96, 0x96, 0x69, 0x69, 0x96, 0x96,
...
static uint8_t __not_in_flash("audio") encode_bch_3(const uint8_t *p)
{
...
static uint8_t __not_in_flash("audio") encode_bch_7(const uint8_t *p)
{
    uint8_t v = bch_table[p[0]];
    v = bch_table[p[1] ^ v];
    v = bch_table[p[2] ^ v];
...
static void __not_in_flash("audio") compute_header_parity(hstx_packet_t *p)
{
    p->header[3] = encode_bch_3(p->header);
}

static void __not_in_flash("audio") compute_subpacket_parity(hstx_packet_t *p, int idx)
{
    p->subpacket[idx][7] = encode_bch_7(p->subpacket[idx]);
}

static void __not_in_flash("audio") compute_all_parity(hstx_packet_t *p)
{
...
void __not_in_flash("audio") hstx_packet_init(hstx_packet_t *packet)
{
...
int __not_in_flash("audio") hstx_packet_set_audio_samples(hstx_packet_t *packet, const audio_sample_t *samples, int num_samples,
                                  int frame_count)
{
    hstx_packet_init(packet);
    if (num_samples < 1)
...
static inline uint32_t __not_in_flash("audio") make_hstx_word(uint16_t lane0, uint16_t lane1, uint16_t lane2)
{
    return (lane0 & 0x3FF) | ((lane1 & 0x3FF) << 10) | ((lane2 & 0x3FF) << 20);
}

static void __not_in_flash("audio") encode_header_to_lane0(const hstx_packet_t *packet, uint16_t *lane0, int hv, bool first_packet)
{
    int hv1 = hv | 0x08;
    if (!first_packet)
        hv = hv1;
...
static void __not_in_flash("audio") encode_subpackets_to_lanes(const hstx_packet_t *packet, uint16_t *lane1, uint16_t *lane2)
{
    for (int i = 0; i < 8; i++) {
...
void __not_in_flash("audio") hstx_encode_data_island(hstx_data_island_t *out, const hstx_packet_t *packet, bool vsync_active, bool hsync_active)
{
