...
int parse_nes_cart_from_buffer(Cartridge* cart, const uint8_t* data, uint32_t size, Cpu6502* cpu, Ppu2C02* ppu)
{
	uint8_t header[16];
	uint8_t mapper;
	uint32_t offset = 0;
	...
	if (size < (16 * KiB - 1)) {
		fprintf(stderr, "Error: ROM buffer is too small.\n");
		return 8;
	}
	...
	if (!memcmp(header, "NES\x1A", 4)) {
		cart->header = INES;
		if (header[9] & INES_BAD_HEADER_9_MASK) {
			cart->header = BAD_INES;
		}
		if ((header[7] & NES2_IDENTIFIER_MASK) == NES2_IDENTIFIER) {
			cart->header = NES_2;
		}
	...
	if (cart->header == HEADERLESS) {
		fprintf(stderr, "Error: unrecognised header in buffer.\n");
		return 8;
	}
	...
	cart->prg_rom.size = 16 * (KiB) * header[4];
	cart->chr_rom.size = 8  * (KiB) * header[5];
	cart->chr_ram.size = 8  * (KiB) * !header[5];
	...
	if (header[6] & NON_VOLATILE_MEM_MASK) {
		cart->non_volatile_mem = true;
	}
	...
	if (cart->header == NES_2) {
		...
		cpu->cpu_mapper_io->mapper_number |= (header[8] & 0x0F) << 8;
		cart->prg_ram.size = 0;
		unsigned shift_count = header[10] & VOLATILE_RAM_SHIFT_MASK;
		if (shift_count) { cart->prg_ram.size = 64 << shift_count; }
		cart->chr_ram.size = 0;
	...
	if (cart->trainer.size) {
		cart->trainer.data = malloc(cart->trainer.size);
		if (!cart->trainer.data) { return 8; }
		memcpy(cart->trainer.data, data + offset, 512);
		offset += 512;
	}

	/* Load PRG ROM */
	cart->prg_rom.data = malloc(cart->prg_rom.size);
	if (!cart->prg_rom.data) { return 8; }
	if (offset + cart->prg_rom.size > size) {
		fprintf(stderr, "Error: ROM buffer too small for PRG ROM.\n");
		return 8;
	}
	memcpy(cart->prg_rom.data, data + offset, cart->prg_rom.size);
	offset += cart->prg_rom.size;

	/* Load CHR ROM */
	if (cart->chr_rom.size) {
		cart->chr_rom.data = malloc(cart->chr_rom.size);
		if (!cart->chr_rom.data) { return 8; }
		if (offset + cart->chr_rom.size > size) {
			fprintf(stderr, "Error: ROM buffer too small for CHR ROM.\n");
			return 8;
		}
		memcpy(cart->chr_rom.data, data + offset, cart->chr_rom.size);
		offset += cart->chr_rom.size;
	}

	/* Allocate CHR RAM */
	if (cart->chr_ram.size) {
		cart->chr_ram.data = calloc(cart->chr_ram.size, sizeof(uint8_t));
		if (!cart->chr_ram.data) { return 8; }
	}
